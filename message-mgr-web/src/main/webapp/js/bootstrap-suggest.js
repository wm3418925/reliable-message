...
(function (factory) {
    ...
    function setValue($input, keywords, options) {
        ...
        $input.trigger('onSetSelectValue', [keywords, (options.data.data || options._lastData.data)[keywords.index]]);
    }
    /**
     * 添加事件触发
     * wmf 2016/12/2 10:38
     */
    function addClick($input, keywords, options) {
        $input.trigger('onAddClick', [keywords, (options.data.data || options._lastData.data)[keywords.index]]);
    }
    function removeClick($input, keywords, options) {
        $input.trigger('onRemoveClick', [keywords, (options.data.data || options._lastData.data)[keywords.index]]);
    }
    /**
     * 调整选择菜单位置
     * @param {Object} $input
    ...
    function adjustDropMenuPos($input, $dropdownMenu, options) {
        ...
        if (options.autoDropup) {
            console.log('options.autoDropup');
            setTimeout(function() {
                if ( // 自动判断菜单向上展开
    ...
    function refreshDropMenu($input, data, options) {
        ...
        if (
            options._lastData &&
            JSON.stringify(options._lastData.data) === JSON.stringify(data.data) &&
            $dropdownMenu.find('tr').length === data.data.length
        ) {
            $dropdownMenu.show();
        ...
        for (i = 0; i < len; i++) {
            index = 0;
            tds = [];
            idValue = data.data[i][options.idField] || '';
            keyValue = data.data[i][options.keyField] || '';

            for (j in data.data[i]) {
                // 标记作为 value 和 作为 id 的值
                if (!keyValue && options.indexKey === index) {
                    keyValue = data.data[i][j];
                }
                if (!idValue && options.indexId === index) {
                    idValue = data.data[i][j];
                }
                ...
                tds.push('<td data-name="', j, '">', data.data[i][j], '</td>');
            }
            tds.push('<td ><a href="#">添加</a>&nbsp;&nbsp;&nbsp;<a href="#">移除</a></td>');
            html.push('<tr data-index="', (data.data[i].__index || i), '" data-id="', idValue,
                ...
                if (!keyword) {
                    ...
                    for (i = 0; i < len; i++) {
                        for (key in data.data[i]) {
                            if (
                                data.data[i][key] &&
                                isInWord(keyword, key, data.data[i][key] + '', options)
                            ) {
                                filterData.data.push(data.data[i]);
                                filterData.data[filterData.data.length - 1].__index = i;
                                break;
                            }
                        }
                    }
    ...
    var defaultOptions = {
        ...
        listHoverStyle: 'background: #EEDFCC;', // 提示框列表鼠标悬浮的样式
        ...
        init: function(options) {
            ...
            return self.each(function() {
                ...
                $dropdownMenu.on('mouseenter', function() {
                    ...
                    .on('click', 'tbody tr a:eq(0)', function() {
                        addClick($input, getPointKeyword($(this).parent().parent()), options);
                    }).on('click', 'tbody tr a:eq(1)', function() {
                    removeClick($input, getPointKeyword($(this).parent().parent()), options);
                });
            });
        },
        show: function() {
